% Run our graph interpolation model until the specified graph distance to the target graph is reached.
...
function [ac,gc,steps] = dgm(B,A,dtarget,slowness,dtrigger,period)
    n = size(A,1);
    ...
    symgraph = graph + graph';
    deg = sum(symgraph, 2); %Determine node degrees
    cn = diag(symgraph*triu(symgraph)*symgraph); %Number of triangles for each node
    %The local clustering coefficient of each node
    c = zeros(size(deg));
    c(deg > 1) = 2 * cn(deg > 1) ./ (deg(deg > 1).*(deg(deg > 1) - 1)); 
    ac = mean(c);
    dummy = sum(cn) / sum(triu(symgraph*symgraph,1),'all');
    dummy(isnan(dummy)) = 0;
    gc = dummy;
